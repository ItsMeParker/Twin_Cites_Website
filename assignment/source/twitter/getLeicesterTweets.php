<html>
  <head>
    ...
    <body class="leicesterBG">
      <section class="tweetsCol">
        <p class="articleTitle">Ten most recent tweets from Leicester and surroundings</p>
        ...
        * Geocode = latitude,longitude, radius */
        ...
        $resType = "recent";
        $getfield = "?q=&geocode=52.637,-1.139,5km&count={$tweetCount}&result_type={$resType}&include_entities=true";
        $requestMethod = 'GET';
      ...
      <p class="articleTitle">Five popular and recent tweets with #Leicester</p>
      <article id="fiveHashes">
        <?php
        $tweetCount = "5";
        $resType = "mixed";
        $getfield = "?q=%23Leicester&count={$tweetCount}&result_type={$resType}&include_entities=true";
        $requestMethod = 'GET';
        $twitter = new TwitterAPIExchange($settings);
        $data=$twitter->setGetfield($getfield)
                     ->buildOauth($url, $requestMethod)
